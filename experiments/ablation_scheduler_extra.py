#!/usr/bin/env python3
"""Extra scheduler runs: 4 more per config (total n=7)."""
...
def main():
    ...
    print(f"Scheduler extra runs: {len(CONFIGS)} configs x {EXTRA_RUNS} runs = {total} total")
    ...
    for cfg_name in CONFIGS:
        for r in range(4, 4 + EXTRA_RUNS):
            ...
            if result.status == "ok":
                print(f"-> score={result.final_score:.0f} ({result.elapsed:.1f}s)")
            else:
                print(f"-> FAILED")
    ...
    raw_path = results_dir / "ablation_scheduler_extra_raw.json"
    raw_path.write_text(json.dumps([asdict(r) for r in results], indent=2))
    print(f"Raw results: {raw_path}")

    # Combined summary with original runs
    existing = results_dir / "ablation_scheduler_raw.json"
    all_runs = {}
    if existing.exists():
        for r in json.loads(existing.read_text()):
            if r["status"] == "ok":
                all_runs.setdefault(r["config"], []).append(r)
    for r in results:
        if r.status == "ok":
            all_runs.setdefault(r.config, []).append(asdict(r))

    print(f"\n## Scheduler combined (n=7 per config):")
    print(f"  {'config':16s} | {'n':>2s} | {'mean':>8s} | {'std':>8s} | {'Δ%':>7s}")
    print(f"  {'─'*16} | {'──':>2s} | {'────────':>8s} | {'────────':>8s} | {'───────':>7s}")
    for cfg_name in CONFIGS:
        runs = all_runs.get(cfg_name, [])
        if not runs:
            continue
        ...
        d_pct = (baseline - s_mean) / baseline * 100 if baseline else 0
        print(f"  {cfg_name:16s} | {len(runs):2d} | {s_mean:8.0f} | {s_std:8.0f} | -{d_pct:.1f}%")
