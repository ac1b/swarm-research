...
\begin{abstract}
We present \textsc{SwarmResearch}, a framework for automated code optimization that combines multiple LLM agents with tree search and a shared memory board.
Given a target program and a numeric evaluation function, \textsc{SwarmResearch} iteratively generates code modifications through diverse agents---each with a distinct optimization strategy and sampling temperature---that share findings via a persistent board.
...
Ablations over 100 runs show that multi-agent diversity contributes up to $2.8\times$ larger gains than single-agent search, and tree-based backtracking enables perfect scores in 57\% of runs versus 0\% without it on the hardest benchmark (Cohen's $d{=}0.53$).
...
\textsc{SwarmResearch} addresses these limitations through three mechanisms:
...
These approaches use flat populations; \textsc{SwarmResearch} maintains a \emph{structured search tree} enabling targeted backtracking to promising states.
...
These systems optimize for \emph{correctness} through role specialization; \textsc{SwarmResearch} optimizes a \emph{numeric objective} with strategy-diverse agents sharing results through a common board.
...
\multirow{3}{*}{TSP\,$\downarrow$}
  ...
  & No-Backtrack    & 592.7 & $497.5 \pm 4.5$           & $-16\%$ \\
...
\textsc{SwarmResearch} combines multi-agent diversity, shared memory, and tree-based backtracking for LLM-driven code optimization. Across four benchmarks, it achieves up to $+282\%$ improvement, with ablations confirming that each component contributes meaningfully on complex tasks. The framework is task-agnostic---any program with a numeric evaluation function can be optimized---and supports both sequential and parallel execution. Code and benchmarks are publicly available.\footnote{Repository URL withheld for anonymous review.}
...
No-Backtrack  & 493.4 & 502.4 & 496.9 & 497.5 & 4.5  \\
